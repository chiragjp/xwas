...
```{r}
glucose.all <- xwas(data=dat, depvar="LBXGLU", adjvars=c("RIDAGEYR", "female"))

glucose.all.adjust <- p.adjust(unlist(lapply(glucose.all, function(x) {return(x[2,]$"Pr(>|t|)")} )))
...
svy.glucose.all.adjust <- p.adjust(unlist(lapply(svy.glucose.all, function(x) {return(x[2,]$"Pr(>|t|)")} )))
...
Side-by-side comparison of non-survey and survey analysis for glucose vs BMI.

```{r}
par(mfrow=c(1,2))
manhattan(glucose.all.sig, nlabels=3, main="Non-svy Glucose vs BMI")
manhattan(svy.glucose.all.sig, nlabels=3, main="Svy Glucose vs BMI")
par(mfrow=c(1,1))
```
